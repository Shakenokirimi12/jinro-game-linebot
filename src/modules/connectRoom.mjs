...
export async function connectRoom(data, request, env) {
    ...
    try {
        if (origintype == "user") {
            ...
            const { results: conncetDestination } = await env.D1_DATABASE.prepare(
            ...
            ).bind(Number(conncetDestination[0].connection_Count) + 1, room_Code).run();

            return [{ "type": "text", "text": "ルーム" + room_Code + "に参加しました。" }];ï
        }
        else {
